...
int main(int argc, const char * argv[])
{
    ...
    int nOST=0, nDAA=0, nReinfected, inIncrease, nDailyOST=0, nDaily=0, nOSTDailyCap=0, nDailyCap=0, dailyInPop=0, hcvAntibody=0, hcvRNA=0, aveLengthStay=0, NCbyOST=0;
	gsl_rng *r;
    ...
    while(currDay!=daysSim+1){
        ...
        while(target!=NULL){
            ...
            if(target->group>=0&&target->group<=5){ //if at-risk, set probabilities accordingly
                ...
                probEvents[21]=0.0;//probHCVdeath(&r, &target);//*rate of HCV-related death
                //scenraios
                probEvents[22]=probOSTstart(&r, &target, nOST, nDailyOST, nOSTDailyCap, currDay);//*rate of commencing OST
            				 ...
            				 switch(iCrawl){
			                   case 0: //natural death
                    			...
                    			case 21: //DEATH BY HCV
                        			printf("case 21\n");
									if(target->risk==1||target->risk==2||target->risk==3||target->risk==7||target->risk==8||target->risk==9){
                            			deadHCVPris++;
                        			}else if(target->placeInfected==4){
                            			deadHCVCom++;
                        			}
...
*/            
        ...
        fprintf(fw, "%d\n", NCbyOST); //number of those infected with HCV upon entry
        printf("DAYS:                %d\n", currDay);
        newCases=0;
