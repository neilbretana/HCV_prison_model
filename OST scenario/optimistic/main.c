...
double probOSTstart(gsl_rng **r, sIndiv **pTarget, int nOST, int nDailyOST, int nOSTDailyCap); //22
    ...
    while(currDay!=daysSim+1){
        ...
        while(target!=NULL){
            ...
            if(target->group>=0&&target->group<=5){ //if at-risk, set probabilities accordingly
                ...
                //scenraios
                probEvents[22]=probOSTstart(&r, &target, nOST, nDailyOST, nOSTDailyCap);//*rate of commencing OST
