...
double probOSTstart(gsl_rng **r, sIndiv **pTarget, int nOST, int nDailyOST, int nOSTDailyCap, int currDay); //22
...
int main(int argc, const char * argv[])
{
    ...
    int nNSP=0, nOST=0, nDAA=0, nReinfected, inIncrease, nDailyNSP=0, nDailyOST=0, nDaily=0, nNSPDailyCap=0, nOSTDailyCap=0, nDailyCap=0, dailyInPop=0, hcvAntibody=0, hcvRNA=0, aveLengthStay=0, NCbyNSP=0;
	gsl_rng *r;
    ...
    while(currDay!=daysSim+1){
        ...
        while(target!=NULL){
            ...
            if(target->group>=0&&target->group<=5){ //if at-risk, set probabilities accordingly
                ...
                probEvents[18]=probInfectIndiv(&target, currDay);//*rate of infecting susceptible individuals //PROBABILITY OF SHARING
                ...
                probEvents[21]=probNSPstart(&r, &target, nNSP, nDailyNSP, nNSPDailyCap, currDay);//probHCVdeath(&r, &target);//*rate of HCV-related death
                //scenraios
                probEvents[22]=0.0;//probOSTstart(&r, &target, nOST, nDailyOST, nOSTDailyCap, currDay);//*rate of commencing OST
                ...
                probEvents[21]=probNSPstart(&r, &target, nNSP, nDailyNSP, nNSPDailyCap, currDay);//*rate of HCV-related death
                probEvents[22]=0.0;//probOSTstart(&r, &target, nOST, nDailyOST, nOSTDailyCap, currDay);//*rate of commencing OST
                probEvents[23]=0.0;//probOSTstop(&r, &target);//*rate of stopping OST
                probEvents[24]=probDAAstart(&r, &target, nDAA, currDay, nDaily, nDailyCap);//*rate of commencing DAA treatment
                probEvents[25]=probDAAstop(&r, &target);//*rate of stopping DAA treatment
