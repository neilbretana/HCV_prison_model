...
void newIndiv (int *idGlobal, int *HCVentry, int *HCVentryAb, int currDay, sIndiv **pHeadCopy, sIndiv **pTailCopy, gsl_rng **r, int nEvents, int (*pLocArray2)[ROWPRIS][COLCTR], int prison, int category){
    sIndiv *obj;
    int objAge, objLoc, objMeta, binIDU, binATSI, binHCV, binPREV, binInjFrq, binOpd, binSha, binShaFrq, binEverIDU, categoryFlag=0, tRisk;
            ...
            if(binIDU==1){ //IDU
                ...
                if(binATSI==1){
                    ...
                    if(binHCV==1){ //IDU ATSI HCV
                        ...
                        if(currDay<4381){
                            probsRisk[0]=1-0.75;//1-0.80; //viraemic
                            probsRisk[1]=0.75;//0.80; //previously exposed
                        }else if(currDay>4380&&currDay<4746){ //2018
                            probsRisk[0]=1-0.704247;//1-0.80; //viraemic
                            probsRisk[1]=0.704247;//0.80; //previously exposed
                        }else if(currDay>4745&&currDay<5111){ //2019
                            probsRisk[0]=1-0.657472907;//1-0.80; //viraemic
                            probsRisk[1]=0.657472907;//0.80; //previously exposed
                        }else if(currDay>5110&&currDay<5476){ //2020
                            probsRisk[0]=1-0.60969732;//1-0.80; //viraemic
                            probsRisk[1]=0.60969732;//0.80; //previously exposed
                        }else if(currDay>5475&&currDay<5841){ //2021
                            probsRisk[0]=1-0.560949364;//1-0.80; //viraemic
                            probsRisk[1]=0.560949364;//0.80; //previously exposed
                        }else if(currDay>5840&&currDay<6206){ //2022
                            probsRisk[0]=1-0.511269158;//1-0.80; //viraemic
                            probsRisk[1]=0.511269158;//0.80; //previously exposed
                        }else if(currDay>6205&&currDay<6571){ //2023
                            probsRisk[0]=1-0.460705646;//1-0.80; //viraemic
                            probsRisk[1]=0.460705646;//0.80; //previously exposed
                        }else if(currDay>6570&&currDay<6936){ //2024
                            probsRisk[0]=1-0.409315093;//1-0.80; //viraemic
                            probsRisk[1]=0.409315093;//0.80; //previously exposed
                        }else if(currDay>6935&&currDay<7301){ //2025
                            probsRisk[0]=1-0.357160773;//1-0.80; //viraemic
                            probsRisk[1]=0.357160773;//0.80; //previously exposed
                        }else if(currDay>7300&&currDay<7666){ //2026
                            probsRisk[0]=1-0.3043126;//1-0.80; //viraemic
                            probsRisk[1]=0.3043126;//0.80; //previously exposed
                        }else if(currDay>7665&&currDay<8031){ //2027
                            probsRisk[0]=1-0.250846959;//1-0.80; //viraemic
                            probsRisk[1]=0.250846959;//0.80; //previously exposed
                        }else if(currDay>8030&&currDay<8396){ //2028
                            probsRisk[0]=1-0.196847056;//1-0.80; //viraemic
                            probsRisk[1]=0.196847056;//0.80; //previously exposed
                        }else if(currDay>8395&&currDay<8761){ //2029
                            probsRisk[0]=1-0.146196102;//1-0.80; //viraemic
                            probsRisk[1]=0.146196102;//0.80; //previously exposed
                        }else{ //if(currDay>8760&&currDay<9125){ //2030
                            probsRisk[0]=1-0.105251248;//1-0.80; //viraemic
                            probsRisk[1]=0.105251248;//0.80; //previously exposed
                        }
...
double probInfectIndiv(gsl_rng **r, sIndiv **pTarget, int currDay){ //PROBABILITY OF SHARING
    ...
    if(current->OST==2){
        ...
        if(current->risk==2||current->risk==5){ //IDU LD
            ...
            pTemp = 0.144375-(0.144375*pChange);//0.159375;//0.044375;
            if(current->NSP==1){
                pTemp = pTemp-(pTemp*.25);
            }
            return pTemp;
            ...
            pTemp = 0.26004902-(0.26004902*pChange);//0.2654902;//0.16004902;
            if(current->NSP==1){
                pTemp = pTemp-(pTemp*.25);
            }
            return pTemp;
        ...
        if(current->risk==2||current->risk==5){ //IDU LD
            ...
            //return 1.3858; //prog
            //return 0.044375; //Rate from Brigid HITS
            pTemp = 0.144375-(0.144375*.25);
            if(current->OST==2){
                pChange=gsl_ran_flat(*r, .55, .75); //pChange for OST == 2
                pTemp = pTemp-(pTemp*pChange);
            }
            return pTemp;
        }else if(current->risk==8||current->risk==11){ //IDU DM
            //return 0.1415705635; //cons/int
            //return 0.249; //prog
            //return 0.16004902; //Rate from Brigid HITS
            pTemp = 0.26004902-(0.26004902*.25);//0.2654902;//0.16004902;
            if(current->OST==2){
                pChange=gsl_ran_flat(*r, .55, .75); //pChange for OST == 2
                pTemp = pTemp-(pTemp*pChange);
            }
...
double probOSTstart(gsl_rng **r, sIndiv **pTarget, int nOST, int currDay, int nDailyOST, int nOSTDailyCap){ //Probability to start DAA therapy (only if infected)
    sIndiv *current;
    current=*pTarget;
...
double probNSPstart(gsl_rng **r, sIndiv **pTarget, int nNSP, int currDay, int nDailyNSP, int nNSPDailyCap){ //Probability to start DAA therapy (only if infected)
    sIndiv *current;
    current=*pTarget;
    
    if(currDay>4380){
        //if(current->risk==1||current->risk==2||current->risk==3||current->risk==4||current->risk==5||current->risk==6||current->risk==7||current->risk==8||current->risk==9||current->risk==10||current->risk==11||current->risk==12){ // only to opioid users
        if(current->everIDU==1){
            //if(nNSP>0){
            //if(nDailyNSP<nNSPDailyCap){
            if(current->NSP==0){ //not yet on OST
                return 0.9;
            }else //not infected
                return 0.0;
            //}
...
double probNSPstop(gsl_rng **r, sIndiv **pTarget){ //Probability to start DAA therapy (only if infected)
    sIndiv *current;
    current=*pTarget;
    
    if(current->NSP==1){ //on NSP assume 100% retention in prison
        return 0.0;
    }else //not infected
        return 0.0;
}
